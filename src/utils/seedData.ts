...
// Embedded data.json content since it's outside src
const SEED_DATA = {
  "boards": [
    {
...
export async function seedDemoData(userId: string) {
  const loadingToast = toast.loading('Seeding data from data.json...');

  try {
    for (const board of SEED_DATA.boards) {
      // Create Board Structure (columns without tasks)
      const boardData = {
        name: board.name,
        columns: board.columns.map(col => ({
          name: col.name,
          tasks: [] // Initially empty, will populate via createTask
        }))
      };

      const newBoard = await boardService.createBoard(userId, boardData);
      
      if (!newBoard || !newBoard.id) {
        console.error(`Failed to create board: ${board.name}`);
        continue;
      }
      ...
      for (const column of board.columns) {
        for (const task of column.tasks) {
          // If status is empty/missing in JSON, fallback to column name or 'Todo'
          ...
          await boardService.createTask(newBoard.id, userId, taskData);
        }
      }
    }

    toast.success('Data seeded successfully!', { id: loadingToast });
    return true;
